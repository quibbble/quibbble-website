    ...
    return (
        <div className="w-full h-screen flex items-center justify-center">
            <div className="flex flex-col w-full h-screen items-center justify-center opacity-0 animate-fade fill-mode-forwards">
                <div className="m-4 flex flex-col justify-center px-8 py-12 bg-dark-900 rounded-3xl">
                    <h1 className={`max-w-96 text-white mb-6 text-4xl drop-shadow-m font-lobster`}>Change your name</h1>
                    <input className="bg-dark-500 text-white focus:outline focus:outline-2 outline-yellow rounded-md p-2" type="text" value={ name } onChange={e => setName(e.target.value)}/>
                    <Link className="group mt-4 font-bold flex items-center justify-center " to={`/`} state={{ from: location.pathname }} >
                        ...
                        <p className="ml-1 text-slate group-hover:text-yellow transition ease-in-out duration-500">go home</p>
                    </Link>
                </div>
            </div>
        </div>
